...
Module.addOnPostRun(() => {
  ...
  setInterval(() => {
    ...
    for (const strip of strips) {
      ...
      const colorVector = Module.Render(
        animationIndex,
        stripIndex,
        ms,
        ledCount
      ...
      colors.forEach((color, index) =>
        updateLed(stripIndexLocal, index, color)
      );
      stripIndex++;
    }
  }, 1000 / 30);
...
const mapPosition = (x: number, y: number): { left: string; top: string } => {
  const left =
    ((x - limits.minX) / (limits.maxX - limits.minX)) * (boxSize.width - 10);
  const top =
    boxSize.height -
    10 -
    ((y - limits.minY) / (limits.maxY - limits.minY)) * (boxSize.height - 10);
